...
import { getTokenBalanceAndSymbol, getAccountBalance, getTokenExchangeAddressFromFactory } from '../utils/ethers';
...
export async function getReserves(tokenAddress: string, networkId: number) {
...
export async function getLPTokenAmount(tokenAddress: string, accountAddress: string, networkId: number) {
    const exchangeAddress = await getTokenExchangeAddressFromFactory(tokenAddress, networkId);
    const tokenReserve = (await getTokenBalanceAndSymbol(accountAddress, exchangeAddress)).balance;
    return tokenReserve;
}

export async function getOutputAmount(inputAmount: BigNumber, inputReserve: BigNumber, outputReserve: BigNumber) {
